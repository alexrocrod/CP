...
__constant__ int dev_ws;
__constant__ int dev_thres;
...
__global__ void kernel_Harris(pixel_t *dev_idata, pixel_t *dev_odata)
{
    int Ix, Iy;   // gradient in XX and YY
    int R;        // R metric
    ...
    // fade image
    dev_odata[i*dev_w+j]=dev_idata[i*dev_w+j]/4;

    if((i>= dev_ws + 1) && (i < dev_h-dev_ws-1) && (j >= dev_ws + 1) && (j < dev_w-dev_ws-1)) // only if valid interior region
    {
        for(int k=-dev_ws; k<=dev_ws; k++) //height window
        {
            for(int l=-dev_ws; l<=dev_ws; l++) //width window
            {
                Ix = ((int)dev_idata[(i+k-1)*dev_w + j+l] - (int)dev_idata[(i+k+1)*dev_w + j+l])/32;         
                Iy = ((int)dev_idata[(i+k)*dev_w + j+l-1] - (int)dev_idata[(i+k)*dev_w + j+l+1])/32;          
                sumIx2 += Ix*Ix;
                sumIy2 += Iy*Iy;
                sumIxIy += Ix*Iy;
            }
        ...
        // is a corner
        if(R > dev_thres){
            dev_odata[i*dev_w+j]=MAX_BRIGHTNESS;
        }
                  ...
                  pixel_t * h_odata)
{
    ...
    cudaMemcpyToSymbol(dev_w, &w, sizeof(int));
    cudaMemcpyToSymbol(dev_h, &h, sizeof(int));
    cudaMemcpyToSymbol(dev_ws, &ws, sizeof(int));
    cudaMemcpyToSymbol(dev_thres, &threshold, sizeof(int));

    // copy image to device (CPU->GPU)
    cudaMemcpy(dev_idata, h_idata, data_size, cudaMemcpyHostToDevice);

    // Run corner detetion on GPU
    kernel_Harris<<<dimGrid, dimBlock>>>(dev_idata, dev_odata);
...
int main( int argc, char** argv) 
{
    ...
    char *fileIn        = (char *)"chess.pgm",
         *fileOut       = (char *)"resultCudaConst.pgm",
         *referenceOut  = (char *)"referenceCudaConst.pgm";
