...
texture<int, cudaTextureType2D, cudaReadModeElementType> textureRef;
...
__global__ void kernel_Harris(const int w, const int h, const int ws, const int threshold,
                                 pixel_t *dev_odata)
{
    int Ix, Iy;   // gradient in XX and YY
    int R;        // R metric
    ...
    dev_odata[i*w+j]=tex2D(textureRef,i,j)/4;
    ...
    if((i>= ws + 1) && (i < h-ws-1) && (j >= ws + 1) && (j < w-ws-1)) // only if valid interior region
    {
        for(int k=-ws; k<=ws; k++) //height window
        {
            ...
            {
                Ix = (tex2D(textureRef,(i+k-1), j+l) - tex2D(textureRef,(i+k+1),j+l))/32;         
                Iy = (tex2D(textureRef,(i+k), j+l-1) - tex2D(textureRef,(i+k),j+l+1))/32; 
                sumIx2 += Ix*Ix;
                sumIy2 += Iy*Iy;
                sumIxIy += Ix*Iy;
            }
                  ...
                  pixel_t * h_odata)
{
    ...
    cudaArray *cu_array;
    cudaMallocArray(&cu_array,&channelDesc, w, h);

    // copy image to texture memory (CPU->GPU)
    cudaMemcpy2DToArray(cu_array,0,0,h_idata,w*sizeof(pixel_t),w*sizeof(pixel_t),h,cudaMemcpyHostToDevice);
    ...
    kernel_Harris<<<dimGrid, dimBlock>>>(w, h, ws, threshold, dev_odata);
...
int main( int argc, char** argv) 
{
    ...
    char *fileIn        = (char *)"chess.pgm",
         *fileOut       = (char *)"resultCudaTexture.pgm",
         *referenceOut  = (char *)"referenceCudaTexture.pgm";
