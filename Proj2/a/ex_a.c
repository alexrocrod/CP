...
int main(int argc, char *argv[]){
    ...
    if (newid == 0) {
        ...
        for (i=0; i<numprocs-2; i+=2) {
            listnumrows[i] = numrows;
            listnumrows[i+1] = numrows;
        ...
        listtoprows[numprocs-2] = nx - remaining;
        listtoprows[numprocs-1] = nx - remaining;
        
        int listleftcols[numprocs], listnumcols[numprocs];
        for (i=0; i<numprocs; i+=2) {
            listleftcols[i] = 1;
            listnumcols[i] = (ny-2)/2;
            listleftcols[i+1] = (ny-2)/2 + 1;
            listnumcols[i+1] = ny-2 - listnumcols[i];
    ...
    printf("%d %d   mytr %d  mybr %d  mylc %d myrc %d \n", myid, newid, mytoprow, mytoprow+myrows-1, myleftcol, myleftcol + mycols -1);
    ...
    // Calcular myf:
    // 1as e ultimas linhas e colunas nao sao usadas,
    // sao alocadas apenas por conveniencia a frente.
    ...
    // Condicoes fronteira em x=-L e x=L:
    //V(-1,y)
    if (newid % 2 == 0) {
    ...
    // Condicoes fronteira em y=L e y=-L:
    if (newid == 0 || newid == 1 ) {
    ...
    for (Jaciter = 0; Jaciter < MAXITER; Jaciter++) {
        ...
        for (i=1; i<myrows+1; i++) {
            for (j=1; j<mycols+1; j++) {
                myVnew[i][j] = 0.25 * (myVold[i-1][j] + myVold[i+1][j] + myVold[i][j-1] + myVold[i][j+1] - h*h*myf[i][j]);
        ...
        if (sqrt(sums_global[0]/sums_global[1]) < TOL) {
            ...
            int localsize[] = {myrows, mycols+1};
            int start_inds[] = {mytoprow, myleftcol - 1 + (newid % 2)};
            if (newid == 0 || newid == 1) {
                localsize[0]++;
                start_inds[0]--;
            }
            ...
            MPI_File pf;
            MPI_File_open(comm2d, "results_a_C.bin", MPI_MODE_CREATE | MPI_MODE_WRONLY,
                MPI_INFO_NULL, &pf);
        ...
        // Se ainda não parou, então fazer trocas:
