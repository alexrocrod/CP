...
int main(int argc, char *argv[]) {
  ...
  printf("myid=%d, newid=%d, nbrbottom=%d, nbrtop=%d, nbrleft=%d, nbrright=%d\n", myid, newid, nbrbottom, nbrtop, nbrleft, nbrright);
  ...
  if (newid == 0) {
    ...
    int nrows = (int)(((double)(ny-2))/((double)dim0) + 0.5);
    ...
    listfirstrow[nprocs-2] = 1 + (dim0-1)*nrows;
    listmyrows[nprocs-2] = ny - 2 - (dim0-1)*nrows;
    listfirstrow[nprocs-1] = 1 + (dim0-1)*nrows;
    listmyrows[nprocs-1] = ny - 2 - (dim0-1)*nrows;

    for (int i=0; i<dim0; ++i) {
        listfirstcol[2*i] = 1;
        listmycols[2*i] = (int)((nx-2)/2);
        listfirstcol[2*i+1] = (int)((nx-2)/2) + 1;
        listmycols[2*i+1] = nx-2 - (int)((nx-2)/2);
    }
